...
pub async fn models(base_url: &str, api_key_name: Option<&str>) -> Result<Vec<Model>> {
    ...
    let mut request = client
        .get(base_url.to_owned() + "/models")
        .query(&params);

    if let Some(api_key_name) = api_key_name {
        request = request.header(AUTHORIZATION, format!("Bearer {}", std::env::var(api_key_name)?))
    } 
...
fn get_default_headers() -> HeaderMap {
    ...
    if provider != Providers::Local {
        headers.insert(
            AUTHORIZATION,
            format!("Bearer {}", std::env::var(provider.api_key_name().unwrap()).unwrap())
                .parse()
                .unwrap(),
        );
    }
